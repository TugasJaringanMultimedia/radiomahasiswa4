...
with app.app_context():
    db.create_all()
    # --- PERBAIKAN: Inisialisasi status siaran langsung dari DB saat aplikasi dimulai ---
...
# Variabel global untuk state siaran saat ini
active_broadcast_id = live_broadcast_on_startup.id if live_broadcast_on_startup else None
start_time_obj = None # Akan diatur saat 'start_broadcast'
file_writer = None
...
def handle_start_broadcast(data):
    ...
    # --- PERBAIKAN: Periksa dan hentikan siaran live yang sudah ada ---
    ...
    if existing_live:
        ...
        # Tutup file_writer jika aktif untuk siaran lama
        if file_writer:
            file_writer.close()
            file_writer = None
    ...
    db.session.commit() # COMMIT DI SINI AGAR ID BARU TERSEDIA
...
def handle_stop_broadcast(data):
    ...
    # --- PERBAIKAN: Pastikan active_broadcast_id TIDAK NULL ---
    if active_broadcast_id is not None and start_time_obj is not None:
        ...
        if broadcast_to_stop:
            ...
            broadcast_to_stop.duration_in_seconds = int(duration.total_seconds()) # Ini yang menyimpan durasi
...
# --- PERBAIKAN: Handler untuk force stop dari penyiar yang refresh halaman ---
...
def handle_force_stop_broadcast():
    ...
    if active_broadcast_id:
        broadcast_to_stop = Broadcast.query.get(active_broadcast_id)
        if broadcast_to_stop:
            broadcast_to_stop.duration_in_seconds = None # Bisa diatur None karena durasi tidak bisa dihitung akurat jika start_time_obj hilang
